    ...
    namespace HistoryTrail {
        ...
        */
        LRESULT HistoryTrailDialog::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
        {
            ...
            HistoryTrailDialog* dialog = reinterpret_cast<HistoryTrailDialog*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));
        ...
        */
        ...
        {
            this->data =
                reinterpret_cast<HistoryTrailData*>(reinterpret_cast<DialogCallArgument*>(lParam)->contextArgument);

            // Checkboxes
            CheckDlgButton(hwnd, IDC_CHECK_AA, *this->data->antiAlias ? BST_CHECKED : BST_UNCHECKED);

            CheckDlgButton(hwnd, IDC_CHECK_FADING, *this->data->fade ? BST_CHECKED : BST_UNCHECKED);

            CheckDlgButton(hwnd, IDC_CHECK_DEGRADING, *this->data->degrade ? BST_CHECKED : BST_UNCHECKED);

            CheckDlgButton(hwnd, IDC_CHECK_FILL, *this->data->filledDots ? BST_CHECKED : BST_UNCHECKED);

            CheckDlgButton(hwnd, IDC_TRAIL_ROTATE, *this->data->rotate ? BST_CHECKED : BST_UNCHECKED);

            // Trail length
            SendDlgItemMessage(hwnd, IDC_SPIN_LENGTH, UDM_SETRANGE, this->minTrailLength, this->maxTrailLength);
            SendDlgItemMessage(
                hwnd,
                IDC_SPIN_LENGTH,
                UDM_SETPOS,
            ...
            // Dot size
            SendDlgItemMessage(hwnd, IDC_DOT_SIZE_SPIN, UDM_SETRANGE, this->minDotSize, this->maxDotSize);
            SendDlgItemMessage(
                hwnd,
                IDC_DOT_SIZE_SPIN,
                UDM_SETPOS,
                NULL,
                this->ValueInRange(*this->data->dotSize, this->minDotSize, this->maxDotSize) ? *this->data->dotSize
                                                                                             : this->defaultDotSize);

            // Trail type - diamond
            SendDlgItemMessage(
                hwnd, IDC_TRAIL_TYPE, CB_INSERTSTRING, this->trailTypeDiamond, reinterpret_cast<LPARAM>(L"Diamond"));

            // Trail type - square
            SendDlgItemMessage(
                hwnd, IDC_TRAIL_TYPE, CB_INSERTSTRING, this->trailTypeSquare, reinterpret_cast<LPARAM>(L"Square"));

            // Trail type - circle
            SendDlgItemMessage(
                hwnd, IDC_TRAIL_TYPE, CB_INSERTSTRING, this->trailTypeCircle, reinterpret_cast<LPARAM>(L"Circle"));

            // Trail type - line
            SendDlgItemMessage(
                hwnd, IDC_TRAIL_TYPE, CB_INSERTSTRING, this->trailTypeLine, reinterpret_cast<LPARAM>(L"Line (NERC)"));

            // Set trail type position
            if (*this->data->type == this->trailTypeDiamond || *this->data->type == this->trailTypeCircle ||
                *this->data->type == this->trailTypeSquare || *this->data->type == this->trailTypeLine) {
                SendDlgItemMessage(hwnd, IDC_TRAIL_TYPE, CB_SETCURSEL, *this->data->type, NULL);
            } else {
                SendDlgItemMessage(hwnd, IDC_TRAIL_TYPE, CB_SETCURSEL, this->trailTypeDiamond, NULL);
            }

            // Max altitude
            SendDlgItemMessage(
                hwnd, IDC_MAX_FILTER_SPIN, UDM_SETRANGE32, this->minAltitudeValue, this->maxAltitudeValue);
            ...
            // Min altitude
            SendDlgItemMessage(
                hwnd, IDC_MIN_FILTER_SPIN, UDM_SETRANGE32, this->minAltitudeValue, this->maxAltitudeValue);
            SendDlgItemMessage(
                hwnd,
                IDC_MIN_FILTER_SPIN,
                UDM_SETPOS32,
        ...
        */
        void HistoryTrailDialog::OnOk(HWND hwnd)
        {
            ...
            *this->data->rotate = IsDlgButtonChecked(hwnd, IDC_TRAIL_ROTATE) == BST_CHECKED;
            ...
            // Length
            int trailLength = SendDlgItemMessage(hwnd, IDC_SPIN_LENGTH, UDM_GETPOS, NULL, NULL);
            ...
            // Dot size
            *this->data->dotSize = SendDlgItemMessage(hwnd, IDC_DOT_SIZE_SPIN, UDM_GETPOS, NULL, NULL);

            // Type
            *this->data->type = SendDlgItemMessage(hwnd, IDC_TRAIL_TYPE, CB_GETCURSEL, NULL, NULL);

            // Min and max altitude
            *this->data->maxAltitude = SendDlgItemMessage(hwnd, IDC_MAX_FILTER_SPIN, UDM_GETPOS32, NULL, NULL);
            *this->data->minAltitude = SendDlgItemMessage(hwnd, IDC_MIN_FILTER_SPIN, UDM_GETPOS32, NULL, NULL);
        }
        ...
        void HistoryTrailDialog::ChoseTrailColour(HWND hwnd)
        {
            ...
            if (ChooseColor(&this->colourStruct) == TRUE) {
                *this->data->colour = this->colourStruct.rgbResult;
                ...
                buttonRect = {
                    clientRectTopLeft.x, clientRectTopLeft.y, clientRectBottomRight.x, clientRectBottomRight.y};
        ...
        */
        ...
        {
            FillRect(drawItem->hDC, &drawItem->rcItem, this->colourSelectorBrush);
        }

...
namespace UKControllerPlugin {
    namespace Hold {
        ...
        */
        void HoldDisplayConfigurationDialog::InitDialog(HWND hwnd, LPARAM lParam)
        {
            ...
            this->configurationItem =
                reinterpret_cast<HoldDisplay*>(reinterpret_cast<DialogCallArgument*>(lParam)->contextArgument);
            ...
            // Max level
            SendDlgItemMessage(hwnd, IDC_HOLD_MAX_SPIN, UDM_SETRANGE32, this->minHoldingLevel, this->maxHoldingLevel);
            SendDlgItemMessage(
                hwnd,
                IDC_HOLD_MAX_SPIN,
                UDM_SETPOS32,
                NULL,
                this->ValueInRange(
                    this->configurationItem->GetMaximumLevel(), this->minHoldingLevel, this->maxHoldingLevel)
                    ? this->configurationItem->GetMaximumLevel()
                    : this->maxHoldingLevel);
            SendDlgItemMessage(hwnd, IDC_HOLD_MAX_SPIN, UDM_SETACCEL, 1, reinterpret_cast<LPARAM>(accelArray));

            // Min level
            SendDlgItemMessage(hwnd, IDC_HOLD_MIN_SPIN, UDM_SETRANGE32, this->minHoldingLevel, this->maxHoldingLevel);
            ...
            SendDlgItemMessage(
                ...
                NULL,
                this->ValueInRange(
                    this->configurationItem->GetMinimumLevel(), this->minHoldingLevel, this->maxHoldingLevel)
                    ? this->configurationItem->GetMinimumLevel()
                    : this->minHoldingLevel);
            SendDlgItemMessage(hwnd, IDC_HOLD_MIN_SPIN, UDM_SETACCEL, 1, reinterpret_cast<LPARAM>(accelArray));
        }
        ...
        */
        ...
        {
            int maximumLevel = SendDlgItemMessage(hwnd, IDC_HOLD_MAX_SPIN, UDM_GETPOS32, NULL, NULL);
            maximumLevel = maximumLevel > this->maxHoldingLevel ? this->maxHoldingLevel : maximumLevel;

            int minimumLevel = SendDlgItemMessage(hwnd, IDC_HOLD_MIN_SPIN, UDM_GETPOS32, NULL, NULL);
        ...
        */
        LRESULT HoldDisplayConfigurationDialog::WndProc(HWND hwnd, UINT msg, WPARAM wParam, LPARAM lParam)
        {
            ...
            HoldDisplayConfigurationDialog* dialog =
                reinterpret_cast<HoldDisplayConfigurationDialog*>(GetWindowLongPtr(hwnd, GWLP_USERDATA));

...
namespace UKControllerPluginTest::Integration {
    ...
    class IntegrationConnectionTest : public Test
    {
        ...
        IntegrationConnectionTest()
            : mockConnection(new testing::NiceMock<MockConnection>()), connection(mockConnection)
        {
        }
    ...
    {
        ON_CALL(*this->mockConnection, Active).WillByDefault(testing::Return(true));
    ...
    {
        ON_CALL(*this->mockConnection, Active).WillByDefault(testing::Return(false));
    ...
    TEST_F(IntegrationConnectionTest, ItSendsAMessage)
    {
        ...
        EXPECT_CALL(*mockConnection, Send(message->ToJson().dump())).Times(1);
    ...
    {
        EXPECT_CALL(*mockConnection, Receive).Times(1).WillOnce(testing::Return(std::queue<std::string>()));
    ...
    TEST_F(IntegrationConnectionTest, ItReturnsParsedMessages)
    {
        ...
        EXPECT_CALL(*mockConnection, Receive).Times(1).WillOnce(testing::Return(messages));

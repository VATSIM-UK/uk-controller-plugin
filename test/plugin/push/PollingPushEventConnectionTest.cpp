...
namespace UKControllerPluginTest {
    namespace Push {
        ...
        {
            public:
            PollingPushEventConnectionTest() : connection(mockApi, mockTaskRunner, collection)
            {
        ...
        {
            nlohmann::json response = {{"event_id", 55}};
        ...
        {
            nlohmann::json response = {{"not_event_id", 55}};
            EXPECT_FALSE(connection.SyncResponseValid(response));
        }
        ...
        TEST_F(PollingPushEventConnectionTest, PluginEventIsValidIfAllPresent)
        {
            nlohmann::json pluginEvent = {
                {"id", 55}, {"event", nlohmann::json{{"data", nlohmann::json{{"foo", "bar"}}}}}};
        ...
        {
            nlohmann::json pluginEvent = {{"id", 55}, {"event", nlohmann::json{{"data", nlohmann::json::array()}}}};
        ...
        {
            nlohmann::json pluginEvent = {{"id", 55}, {"event", nlohmann::json::object()}};
        ...
        {
            nlohmann::json pluginEvent = {{"id", 55}, {"event", nlohmann::json::array()}};
        ...
        {
            nlohmann::json pluginEvent = {{"event", nlohmann::json{{"data", nlohmann::json{{"foo", "bar"}}}}}};
        ...
        {
            EXPECT_CALL(mockApi, SyncPluginEvents()).Times(1).WillOnce(Throw(ApiException("foo")));
        ...
        {
            EXPECT_CALL(mockApi, SyncPluginEvents()).Times(1).WillOnce(Return(nlohmann::json::array()));
        ...
        {
            nlohmann::json syncResponse{{"event_id", 55}};

            EXPECT_CALL(mockApi, SyncPluginEvents()).Times(1).WillOnce(Return(syncResponse));
        ...
        TEST_F(PollingPushEventConnectionTest, ItTriggersASyncedEventWhenSyncComplete)
        {
            ...
            ON_CALL(*this->eventProcessor, GetPushEventSubscriptions).WillByDefault(Return(subs));
        ...
        TEST_F(PollingPushEventConnectionTest, ItDoesntSyncIfSyncInProgress)
        {
            ...
            EXPECT_CALL(mockApi, SyncPluginEvents()).Times(0);
        ...
        TEST_F(PollingPushEventConnectionTest, ItHandlesApiExceptionWhenUpdating)
        {
            ...
            EXPECT_CALL(mockApi, GetLatestPluginEvents(0)).Times(1).WillOnce(Throw(ApiException("foo")));
        ...
        TEST_F(PollingPushEventConnectionTest, ItHandlesApiResponseNotArrayWhenUpdating)
        {
            ...
            EXPECT_CALL(mockApi, GetLatestPluginEvents(0)).Times(1).WillOnce(Return(nlohmann::json::object()));
        ...
        TEST_F(PollingPushEventConnectionTest, ItHandlesApiResponseWherePluginEventIsNotValid)
        {
            ...
            nlohmann::json responseData = nlohmann::json::array(
                {{{"event", nlohmann::json{{"channel", "foo"}, {"data", nlohmann::json{{"bar", "baz"}}}}}}});

            EXPECT_CALL(mockApi, GetLatestPluginEvents(0)).Times(1).WillOnce(Return(responseData));
        ...
        TEST_F(PollingPushEventConnectionTest, ItUpdatesLastEventIdWhenUpdating)
        {
            nlohmann::json response = nlohmann::json::array(
                {{{"id", 3}, {"event", nlohmann::json{{"channel", "foo"}, {"data", nlohmann::json{{"bar", "baz"}}}}}},
                 {{"id", 7},
                  {"event", nlohmann::json{{"channel", "bish"}, {"data", nlohmann::json{{"bash", "bosh"}}}}}}});

            connection.SetSynced();

            EXPECT_CALL(mockApi, GetLatestPluginEvents(0)).Times(1).WillOnce(Return(response));
        ...
        {
            nlohmann::json response = nlohmann::json::array(
                {{{"id", 3}, {"event", nlohmann::json{{"channel", "foo"}, {"data", {{"bar", "baz"}}}}}},
                 {{"id", 7}, {"event", nlohmann::json{{"channel", "bish"}, {"data", {{"bash", "bosh"}}}}}}});
            ...
            std::string firstExpectedMessage = nlohmann::json{{"channel", "foo"}, {"data", {{"bar", "baz"}}}}.dump();
            EXPECT_EQ(firstExpectedMessage, connection.GetNextMessage());

            std::string secondExpectedMessage =
                nlohmann::json{{"channel", "bish"}, {"data", {{"bash", "bosh"}}}}.dump();
        ...
        TEST_F(PollingPushEventConnectionTest, TestItDoesntUpdateIfUpdateInProgress)
        {
            ...
            EXPECT_CALL(mockApi, GetLatestPluginEvents(0)).Times(0);
